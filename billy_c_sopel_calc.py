...
async def c_calc(client, message):
	"""Evaluate some calculation."""
	if len(sh.get_args(message)) == 0:
		await message.channel.send("Nothing to calculate.")
		return
...
async def c_py(client, message):
	"""Evaluate a Python expression."""
	if len(sh.get_args(message)) == 0:
		await message.reply("Need an expression to evaluate")
		return
	...
	if answer:
		...
		await message.reply('Sorry, no result.')
