...
try:
	...
	def warn(text, date=False):
		if date:
			...
			print(text)
	warn("Colorama library not installed, errors won't be colorized")
...
def debug(msg, obj=False):
	if testing:
		if obj != False:
			if obj.guild:
				print((msg + " [author: {}; server: {}; channel: {}; time: {}]".format(str(obj.author), str(obj.guild), str(obj.channel), str(obj.created_at.strftime("%Y-%m-%d %H:%M:%S")))).strip(" "))
			else:
				print((msg + " [author: {}; time: {}]".format(str(obj.author), str(obj.created_at.strftime("%Y-%m-%d %H:%M:%S")))).strip(" "))
		else:
			print(msg)
...
def dump_errlog_msg(content):
	return "Ojoj, chyba serwer mi po śląsku odpowiedział. <@{}> gamoniu sprawdź plik ".format(BOT_OWNERS[0]) + dump_errlog(content)
...
def get_command(msg):
	...
	if msg_strip.startswith(who_prefixes):
		...
		try:
			...
			if msg_strip:
				warn("Something went wrong while recognizing this command: " + msg_strip)
			return None
